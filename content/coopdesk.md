...
attestations fiscale lui permettant de prouver sa participation à la
coopérative. Il veut aussi avoir accès aux informations personnelles que
la coopérative détient à son sujet (adresse, compte bancaire, etc.) afin de
pouvoir rectifier ces derniers en cas d'erreur. Il veut connaitre l'état
de ses parts dans la coopérative (combien de parts, quel type de part,
la date de prise de part, etc.). La possession de parts donne droit au
coopérateur de prendre part active dans les différentes instances de
décisions démocratiques de la coopérative, c'est pourquoi il souhaite
avoir accès aux rapports des différentes assemblées, ainsi qu'aux textes
de règlements.
...
son statut à tout instant, les dates de ses prochains *shifts* de travail
et les dates clés qui modifient son statut (date d'alerte, date de
congé, etc.). Le coopérateur peut autoriser jusqu'à deux personnes à faire
les courses dans le supermarché à sa place, c'est une information à
laquelle le coopérateur doit avoir accès afin de la modifier si
nécessaire.
...
coopérateurs cela demande beaucoup de ressources humaines. La coopérative
préfère mettre ses ressources humaines au travail pour la
...
utilisateurs interagissent avec Odoo via le navigateur internet de leur
poste de travail. Odoo est écrit en Python. Il est constitué d'un noyau,
un *framework*, qui fourni un ORM *(Object-Relational Mapping)*, et des
fonctionnalités de base qui rendent possible la création de modules. Ces
modules sont des petits logiciels à part entière qui viennent se greffer
sur le noyau afin de fonctionner tous ensemble pour fournir un tout
cohérent. Dans sa version Community une série de modules est fournie et
...
coopératives qui ont des besoins similaires. Afin que cela soit possible,
il faut que CoopDesk soit modulable de façon à ce que chaque coopérative
puisse prendre les fonctionnalités qui l'intéressent sans s'encombrer
des fonctionnalités dont elle n'a pas besoin. Il faut aussi que CoopDesk
puisse facilement être complété par d'autres fonctionnalités. Le premier
travail de conception consiste donc à découper CoopDesk en modules,
chaque module faisant un ensemble de tâches indissociables. De manière à
ce que les modules soient le plus possible découplé les uns des autres.
...
ont entre eux. De plus cette figure indique les modules existant
sur lesquelles CoopDesk s'appuie. Les petites boites représentent les
modules existants tandis que les grandes boites représentent les modules
à développer. Les cadres en pointillé regroupent les modules par
thématiques que l'on peut comparer à des applications, car les modules
qui en font partie forment un tout cohérent.  Ce sont ces thématiques
qui serviront de découpage à cet ouvrage.

![Structure et dépendances entre les modules qui composent CoopDesk. En
petit et gris, les modules déjà existant. En grand, les modules à
développer. Les encadrés regroupent les modules par
thématiques.](images/intranet_package.png){#fig:intranet_package
height=150%}
...
Ce module contient toute la gestion de coopérateur et de leurs part dans
une coopérative. C'est dans ce module que l'on trouvera toutes les
informations tel que le nombre de parts, le type de part, la date
d'entrée dans la coopérative, le numéro de coopérateur, le numéro de
compte en banque, etc.

##### Easy My Coop Taxshelter Report

Ce module apporte la gestion du « tax shelter ». Ce dernier est une
attestation qui donne droit en Belgique a une réduction d'impôt pour le
montant investi dans une nouvelle société qui n'a pas encore levé
250 000 euros de capital. Les nouvelles coopératives peuvent faire
profiter leurs coopérateurs de cet avantage.
...
Ce module fourni les fonctionnalités liées à la gestion d'un point de
...
se trouve les *shifts*, les présences et le statut de chaque coopérateur.
C'est aussi via ce module que l'on peut inscrire un coopérateur à un
*shift*.
...
pour être étendu ou modifié. De plus, il gère mal le cas où le
coopérateur est une personne morale telle une ASBL ou une autre
coopérative. Ce module vient donc réorganiser le code afin d'apporter
une meilleure gestion des personnes morales et rendre facilement
modifiable les informations qui sont affichées et modifiées.
...
téléphone et l'adresse électronique restent modifiables par le
...
qui les concernent. Mais aussi tout autre document que la coopérative
voudrait publier.
...
- CoopShift s'est déroulée de septembre à juin avec deux versions
  principale publiées. La première en janvier et la deuxième en avril.
...
volonté sur le web. Pour Odoo les ressources en ligne sont les suivantes :
...
- Le livre *Odoo cookbook* qui fournit des exemples et des explications
  concernant les cas de figure les plus récurant dans la programmation
...
expert Odoo est toujours plus rapide et permets d'éviter de se lancer
...
module dont Odoo dispose. Il faut du temps pour trouver tous les modules
qui interviennent dans une application donnée et comprendre ce que
chacun d'eux apporte. C'est une fois qu'on a compris cela qu'on peut
